...
const privacyPolicyValidation = [
  body("scope")
    .isIn(["global", "client"])
    .withMessage("Scope must be global or client"),
  ...
  body("requiresReacceptance")
    .optional()
    .isBoolean()
    .withMessage("Requires reacceptance must be boolean"),
];
...
const termsOfServiceValidation = [
  body("scope")
    ...
    .withMessage("Scope must be global or client"),
  body("documentType")
    .isIn(["terms_of_service", "service_agreement", "api_agreement", "sla"])
  ...
  body("requiresReacceptance")
    ...
    .isBoolean()
    .withMessage("Requires reacceptance must be boolean"),
  body("appliesTo.users")
    .optional()
    .isBoolean()
    .withMessage("Applies to users must be boolean"),
  body("appliesTo.clients")
    .optional()
    .isBoolean()
    .withMessage("Applies to clients must be boolean"),
  body("appliesTo.admins")
    .optional()
    .isBoolean()
    .withMessage("Applies to admins must be boolean"),
  body("minimumAge")
    .optional()
    .isInt({ min: 0, max: 21 })
    .withMessage("Minimum age must be between 0 and 21"),
];
...
router.get("/security-status", (req, res) => {
  ...
  exec("npm audit --json", { cwd: process.cwd() }, (error, stdout, stderr) => {
    ...
    exec(
      ...
      (updateError, updateStdout, updateStderr) => {
...
router.get("/monitoring-logs", (req, res) => {
  ...
  exec("tail -n 50 /var/log/syslog", (logError, logStdout, logStderr) => {
    ...
    exec(
      ...
      (diskError, diskStdout, diskStderr) => {
        if (diskError) {
          return res.status(500).json({
  ...
  try {
    ...
    const collectionStats = await Promise.all(
      collections.map(async (collection) => {
        ...
        let category = "core";
        if (
          collectionName.includes("Auth") ||
          collectionName.includes("Token")
        ) {
          category = "auth";
        } else if (
          collectionName.includes("Payment") ||
          collectionName.includes("Subscription")
        ) {
          category = "billing";
        } else if (
          collectionName.includes("Log") ||
          collectionName.includes("Activity")
        ) {
          category = "logs";
        }
...
router.get(
  "/database/export/:collectionName",
  authenticateAdmin,
  async (req, res) => {
    try {
      const { collectionName } = req.params;
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${collectionName}_${Date.now()}.json"`,
      );
      res.json(data);
  ...
  async (req, res) => {
    try {
      ...
      await req.admin.logActivity(
        ...
        null,
        {
          collectionsOptimized: optimizationResults.filter(
      ...
      res.json({
        ...
        summary: {
          totalCollections: collections.length,
          optimized: optimizationResults.filter((r) => r.status === "optimized")
            .length,
          failed: optimizationResults.filter((r) => r.status === "failed")
            .length,

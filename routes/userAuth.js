...
router.post("/signup", signupLimiter, validateSignup, async (req, res) => {
  try {
    ...
    // Log activity
    await logUserActivity(newUser._id, "auth", "user_registered", req, {
      email: email.toLowerCase(),
    });
  ...
  try {
    ...
    if (!user.phone) {
      return res.status(400).json({
        success: false,
        message:
          "Phone number not registered. Please add a phone number first.",
      });
    }
    ...
    return res.status(200).json({
      ...
      debug:
        process.env.NODE_ENV === "development" ? { otp: phoneOtp } : undefined,
    });
  } catch (error) {

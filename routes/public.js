...
const shouldRecordImpression = (notification, userId) => {
  ...
  const existingEngagement = notification.userEngagements.find(
    (eng) =>
      eng.userId &&
...
const checkAudienceTargeting = (notification, userId, clientId, user) => {
  ...
  switch (targetAudience) {
    ...
    case "specific_clients":
      if (
        !clientId ||
        !notification.targetedClients ||
        notification.targetedClients.length === 0
      ) {
        return false;
      }
      return notification.targetedClients.some(
    ...
    case "specific_users":
      if (
        ...
        notification.targetedUsers.length === 0
      ) {
        return false;
      }
      return notification.targetedUsers.some(
    ...
    case "test":
      ...
      return (
        process.env.NODE_ENV === "development" || req.query.testMode === "true"
      );
...
const checkPageTargeting = (notification, currentPath, user) => {
  ...
  switch (pageTargeting.type) {
    ...
    case "except_pages":
      if (
        !pageTargeting.exceptPages ||
        pageTargeting.exceptPages.length === 0
      ) {
        return true;
      }
...
const checkRouteConditions = (notification, user) => {
  ...
  if (
    routeConditions.subscriptionStatus &&
    routeConditions.subscriptionStatus.length > 0
  ) {
    if (
      !user ||
      !routeConditions.subscriptionStatus.includes(user.subscription?.status)
    ) {
      return false;
    }
  }
    ...
    const publicSettings = {
      ...
      compliance: {
        ...
        cookieConsent: {
          ...
          privacyPolicyUrl:
            settings.compliance?.cookieConsent?.privacyPolicyUrl,
        },
        dataProcessingAddendumUrl:
          settings.compliance?.dataProcessingAddendumUrl,
      },

      // Feature flags (only public ones)
      featureFlags: settings.featureFlags
        ?.filter(
          (flag) => flag.targetUsers === "all" || flag.targetUsers === "beta",
        )
        .map((flag) => ({
          name: flag.name,
      ...
      defaultPolicySettings: {
        requirePolicyAcceptance:
          settings.defaultPolicySettings?.requirePolicyAcceptance,
        forceReacceptOnUpdate:
          settings.defaultPolicySettings?.forceReacceptOnUpdate,
      },
...
router.get("/privacyPolicy", async (req, res) => {
  try {
    ...
    if (!currentVersion) {
      return res
        .status(404)
        .json({ message: "No published version found for the active policy." });
    }
...
router.get("/termsOfService", async (req, res) => {
  try {
    ...
    if (!terms) {
      return res
        .status(404)
        .json({ message: "No active Terms of Service found." });
    }
    ...
    if (!currentVersion) {
      return res
        ...
        .json({
          message:
            "No published version found for the active Terms of Service.",
        });
    }
  ...
  try {
    ...
    if (token) {
      try {
          ...
          if (req.query.includeUserInfo === "true") {
            try {
              user = await User.findById(userId).select(
                "usertype subscription tier emailVerified",
              );
            } catch (userError) {
              console.log(
                "Could not fetch user details, continuing without...",
              );
            }
          }
        }
        ...
        console.log(
          "Token validation failed, proceeding as anonymous user:",
          tokenError.message,
        );
      }
    }
    ...
    const allNotifications = await NotificationAlert.find(baseQuery).sort({
      priority: -1,
      createdAt: -1,
    });
    ...
    const filteredNotifications = allNotifications.filter((notification) => {
      ...
      if (viewedNotifications.includes(notification._id.toString())) {
        ...
        if (notification.maxShowCountPerUser > 0) {
          const viewCount = viewedNotifications.filter(
            (id) => id === notification._id.toString(),
          ).length;
      ...
      if (userId && notification.showInterval > 0) {
        ...
        if (userEngagement?.viewedAt) {
          const timeSinceLastShow =
            Date.now() - new Date(userEngagement.viewedAt).getTime();
...
router.post("/notification-alerts/:id/impression", async (req, res) => {
  try {
    ...
    const shouldRecord =
      !userId || shouldRecordImpression(notification, userId);
...
router.post("/notification-alerts/:id/click", async (req, res) => {
  try {
    ...
    if (userId) {
      const engagement = notification.userEngagements.find(
        (eng) =>
          eng.userId &&
          eng.userId.toString() === userId.toString() &&
          !eng.clickedAt,
...
router.post("/nav/search/click", async (req, res) => {
  try {
    ...
    try {
      await SearchAnalytics.recordSearchEvent(null, "click", {
        suggestionId,
        query,
        position,
        userType: "public",
...
router.post("/enquiry", enquiryValidation, async (req, res) => {
  try {
    ...
    const { name, email, phone, category, subject, urgency, message } =
      req.body;
    ...
    if (recentEnquiries.length >= 5) {
      return res.status(429).json({
        success: false,
        message:
          "Rate limit exceeded. You can only submit 5 enquiries every 24 hours. Please try again later.",
        limit: 5,
        period: "24 hours",
        nextAvailable: new Date(
          recentEnquiries[0].createdAt.getTime() + 24 * 60 * 60 * 1000,
        ),
      });
    }
  ...
  async (req, res) => {
    try {
      ...
      const enquiries = await PublicEnquiry.find({
        ...
        .select(
          "name email phone category subject urgency status isResolved createdAt updatedAt resolvedAt adminNotes message",
        )
        .sort({ createdAt: -1 })
        .lean();
      ...
      if (!enquiries || enquiries.length === 0) {
        return res.status(404).json({
          success: false,
          message:
            "No enquiries found for the provided email and phone combination",
        });
      }
